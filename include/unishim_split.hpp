...
// #define UNISHIM_PUN_TYPE before including for the callback userdata type to be a pointer to whatever type you want. It's void* by deafult.
...
typedef int (*unishim_callback)(uint32_t codepoint, size_t offset, uint8_t size, UNISHIM_PUN_TYPE * userdata);
...
*/
UNISHIM_DECLARATION_PREFIX int utf8_iterate(uint8_t * utf8, size_t max, unishim_callback callback, UNISHIM_PUN_TYPE * userdata)
{
    if(!utf8)
        return -1;
    ...
    {   
        ...
        if(counter[0] < 0x80)
        {
            if(callback)
            {
                int r = callback(counter[0], i, 1, userdata);
                if(r) return r;
            }
        ...
        else if(counter[0] < 0xE0)
        {
            ...
            if(callback)
            {
                int r = callback(codepoint, i, 2, userdata);
                if(r) return r;
            }
        ...
        else if(counter[0] < 0xF0)
        {
            ...
            if(callback)
            {
                int r = callback(codepoint, i, 3, userdata);
                if(r) return r;
            }
        ...
        else if(counter[0] < 0xF8)
        {
            ...
            if(callback)
            {
                int r = callback(codepoint, i, 4, userdata);
                if(r) return r;
            }
